...
01  MSG-STATUS         PIC XX.
01  EST-STATUS         PIC XX.
...
*> Messaging variables
01  RECIPIENT-USERNAME  PIC X(50).
01  MESSAGE-CONTENT     PIC X(500).
01  MESSAGE-STRING      PIC X(700).
01  TIMESTAMP-DATE      PIC X(8).
01  TIMESTAMP-TIME      PIC X(6).
01  TIMESTAMP-FORMATTED PIC X(19).
01  CONNECTED-OK        PIC X VALUE "N".
...
SEND-MESSAGE-FLOW.
    ...
    READ INFILE
        AT END MOVE "Y" TO EOF
        NOT AT END MOVE IN-REC TO RECIPIENT-USERNAME
    END-READ
    IF EOF = "Y" EXIT PARAGRAPH END-IF

    PERFORM VALIDATE-RECIPIENT-CONNECTION
    IF RECIPIENT-STATUS NOT = "V"
        MOVE VALIDATION-MESSAGE TO OUTPUT-BUFFER
        PERFORM DUAL-OUTPUT
        EXIT PARAGRAPH
    END-IF
...
CHECK-IS-CONNECTED.
    ...
    PERFORM UNTIL JOB-EOF = "Y"
        READ ESTABLISHED-FILE INTO EST-REC
            AT END
                MOVE "Y" TO JOB-EOF
            NOT AT END
                IF (FUNCTION UPPER-CASE(FUNCTION TRIM(ER-USER-A)) = FUNCTION UPPER-CASE(FUNCTION TRIM(AR-USERNAME)) AND
                    FUNCTION UPPER-CASE(FUNCTION TRIM(ER-USER-B)) = FUNCTION UPPER-CASE(FUNCTION TRIM(RECIPIENT-USERNAME))) OR
                   (FUNCTION UPPER-CASE(FUNCTION TRIM(ER-USER-B)) = FUNCTION UPPER-CASE(FUNCTION TRIM(AR-USERNAME)) AND
                    FUNCTION UPPER-CASE(FUNCTION TRIM(ER-USER-A)) = FUNCTION UPPER-CASE(FUNCTION TRIM(RECIPIENT-USERNAME)))
                    MOVE "Y" TO CONNECTED-OK
                    MOVE "Y" TO JOB-EOF
                END-IF
        END-READ
    END-PERFORM
...
GET-CURRENT-TIMESTAMP.
    MOVE FUNCTION CURRENT-DATE(1:8)  TO TIMESTAMP-DATE
    MOVE FUNCTION CURRENT-DATE(9:6)  TO TIMESTAMP-TIME
    MOVE SPACES TO TIMESTAMP-FORMATTED
    *> Format YYYY-MM-DD HH:MM:SS
    MOVE TIMESTAMP-DATE(1:4)  TO TIMESTAMP-FORMATTED(1:4)
    MOVE "-"                 TO TIMESTAMP-FORMATTED(5:1)
    MOVE TIMESTAMP-DATE(5:2)  TO TIMESTAMP-FORMATTED(6:2)
    MOVE "-"                 TO TIMESTAMP-FORMATTED(8:1)
    MOVE TIMESTAMP-DATE(7:2)  TO TIMESTAMP-FORMATTED(9:2)
    MOVE " "                 TO TIMESTAMP-FORMATTED(11:1)
    MOVE TIMESTAMP-TIME(1:2)  TO TIMESTAMP-FORMATTED(12:2)
    MOVE ":"                 TO TIMESTAMP-FORMATTED(14:1)
    MOVE TIMESTAMP-TIME(3:2)  TO TIMESTAMP-FORMATTED(15:2)
    MOVE ":"                 TO TIMESTAMP-FORMATTED(17:1)
    MOVE TIMESTAMP-TIME(5:2)  TO TIMESTAMP-FORMATTED(18:2)
    EXIT PARAGRAPH.
...
SAVE-MESSAGE.
    ...
    STRING
        ...
        FUNCTION TRIM(TIMESTAMP-FORMATTED) DELIMITED BY SIZE
        INTO MESSAGE-STRING
    END-STRING
...
VALIDATE-RECIPIENT-CONNECTION.
    MOVE SPACE TO RECIPIENT-STATUS
    MOVE "N"   TO RECIPIENT-FOUND
    MOVE SPACES TO VALIDATION-MESSAGE

    *> Check if recipient exists in AccountRecords.txt (case-insensitive)
    MOVE "N" TO EOF-ACCT
    OPEN INPUT ACCOUNT-FILE
    PERFORM UNTIL EOF-ACCT = "Y" OR RECIPIENT-FOUND = "Y"
        READ ACCOUNT-FILE
            AT END
                MOVE "Y" TO EOF-ACCT
            NOT AT END
                IF ACCOUNT-REC NOT = SPACES
                   UNSTRING ACCOUNT-REC DELIMITED BY ","
                   INTO FILE-USERNAME FILE-PASSWORD
                   END-UNSTRING
                   IF FUNCTION UPPER-CASE(FUNCTION TRIM(FILE-USERNAME)) =
                      FUNCTION UPPER-CASE(FUNCTION TRIM(RECIPIENT-USERNAME))
                       MOVE "Y" TO RECIPIENT-FOUND
                   END-IF
                END-IF
        END-READ
    END-PERFORM
    ...
    IF RECIPIENT-FOUND NOT = "Y"
        MOVE "U" TO RECIPIENT-STATUS
        MOVE "User not found in your network." TO VALIDATION-MESSAGE
        EXIT PARAGRAPH
    END-IF
    ...
    *> Check connection status
    ...
    IF CONNECTED-OK = "Y"
        MOVE "V" TO RECIPIENT-STATUS
    ELSE
        MOVE "N" TO RECIPIENT-STATUS
        MOVE "You can only send messages to connected users." TO VALIDATION-MESSAGE
    END-IF
    EXIT PARAGRAPH.
